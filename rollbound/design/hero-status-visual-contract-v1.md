...
- The bitmap contains no frame, Level readout, status effect, equipment, text or baked glow.
- Runtime presentation places the transparent bust directly against the shared HUD background. Do not add a square card, diamond backing plate, corner frame or scanline behind it.
...
- Level is a compact `LEVEL` plus number readout in the vitals heading, paired with the next-level reward.
- Level must not overlap the portrait or use a separate card/badge treatment.
- A level increase triggers one short gold stepped pulse. The displayed number always comes directly from `hero.level`.

## HP and XP bars

- Both bars use the same notched dark-plum housing, an 18 px continuous fill and no segment overlay.
- Stable `current / max` values live in their own right-aligned column outside the bar fill; text must never sit on top of the colored fill.
...
- `LEVEL` and `NÆSTE` share the heading above the bars, with the current Level and next upgrade both visible.
...
- Level up: portrait and Level readout receive a stronger gold pulse for approximately 900 ms; XP may pulse simultaneously.
...
- At `≤ 720 px` the unframed portrait and vitals remain side-by-side in an `86 px + flexible` row; equipment, stats and boss readiness continue below.
...
- Real level-up: Level readout changes to `2`, the next threshold becomes `30`, retained XP is truthful and Level/XP feedback activates.
