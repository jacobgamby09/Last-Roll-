...
export function PixelHud({ onOpenInventory, state }: { onOpenInventory?: () => void; state: GameState }) {
  ...
  const nextBonus = CONFIG.levelUpMode === 'rotation' ? PICK_LABEL[rotationPick(hero.level + 1)] : 'Choose 1 of 3';
  ...
  return (
    <section className={`pixel-hud ${feedback.kind ? `has-${feedback.kind}` : ''} ${feedback.xpGain ? 'has-xp' : ''}`} aria-label="Hero status">
      ...
      <div className="pixel-vitals">
        <div className="pixel-vitals-heading">
          ...
          <span className="pixel-next-bonus"><small>NEXT</small><b>{nextBonus}</b></span>
        </div>
        <div className="pixel-vital-row">
          <span className="pixel-vital-label"><ResourceIcon assetId="life" size="mini" /><b>HP</b></span>
          <PixelBar current={hero.hp} feedback={feedback.kind} label={`HP: ${hero.hp} of ${hero.maxHp}`} max={hero.maxHp} previousHp={feedback.previousHp} tone="hp" />
        ...
        <div className="pixel-vital-row">
          ...
          <PixelBar current={hero.xp} feedback={feedback.xpGain ? 'xp' : null} label={`XP: ${hero.xp} of ${xpToNext(hero.level)}`} max={xpToNext(hero.level)} previousHp={feedback.previousHp} tone="xp" />
      ...
      <button
        aria-label={`Open inventory: ${hero.consumables.length} of ${CONFIG.consumableSlots} items`}
        className="pixel-consumable-slots"
        onClick={onOpenInventory}
        type="button"
      >
        <small>ITEMS {hero.consumables.length}/{CONFIG.consumableSlots} · OPEN (I)</small>
        <div>
      ...
      <div className="pixel-stats-grid">
        ...
        <StatBlock icon={<ResourceIcon assetId="gold" size="hud" />} label="GOLD" value={hero.gold} />
      ...
      <div className="pixel-boss-readiness">
        <small>THE BOSS · TILE {CONFIG.trackLength}</small>
