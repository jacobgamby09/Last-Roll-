...
1. Fullscreen scene system + combat screen — decided 2026-09-02: **every interactive phase (combat, post-combat loot, treasure, equipment comparison, shop, level-up choice) renders as a full-screen takeover** over the still-mounted board, built on one shared scene shell (backdrop, top bar with board context "Felt N · …", uniform enter/exit transitions ≤250 ms, skip and reduced-motion rules, timing in one UI constants file). The combat scene is a duel composition (hero left, enemy right, tier-colored name plate) playing back `state.lastCombat` 1:1 — input locked, one click/space skips to payout, reduced-motion jumps to a static result card. Victory payout (XP/gold/drops), the drop Equip/Keep comparison and level-up fanfare play inside the scene's outro; defeat goes to game over inside the scene. The reserved bottom area is the future spell/consumable action bar. Enemy sprites (7: Goblin, Bandit, Ogre, three elites, boss, ~64×80) need a new `combat-sprite-contract-v1.md`; the scene is asset-agnostic with explicit fallback so the sprite batch can be produced in parallel. In the same slice, remove the exact-price UI (combat tile chips, destination HP forecasts, deadly flags, the boss-price HUD panel) and add hover/click enemy inspection showing type + approximate stats **as intervals** (e.g. "HP 20-25 · DMG 7-9") per `AGENTS.md`.
