...
3. Icon batch for the 24 new gear items (Codex track, per `equipment-asset-contract-v1.md`; ids and names in `src/core/items.ts`). The placeholder glyph is the explicit fallback meanwhile.
4. Batch C: consumables (2 slots, 10 items incl. Bombe/Røgbombe/Skæbneterning/Teleport) + the pre-combat beat in the combat scene; shop slots then roll thirds gear/consumable/service. Reaching this completes the playtest gate.
