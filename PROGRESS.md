...
1. Combat screen in the pixel UI — decided 2026-09-02: a **full-screen takeover** (fixed overlay; the board stays mounted underneath). Duel composition (hero left, enemy right, tier-colored name plate, top bar keeps board context "Felt N · Enemy"), playback driven 1:1 by `state.lastCombat` (input locked, one click/space skips to payout, reduced-motion jumps to a static result card, all timing in one UI constants file). Victory payout (XP/gold/drops), the drop equipment Equip/Keep comparison and level-up fanfare play inside the scene's outro; defeat goes to game over inside the scene. The reserved bottom area is the future spell/consumable action bar. Enemy sprites (7: Goblin, Bandit, Ogre, three elites, boss, ~64×80) need a new `combat-sprite-contract-v1.md`; the scene is built asset-agnostic with explicit fallback so the sprite batch can be produced in parallel. In the same slice, remove the exact-price UI (combat tile chips, destination HP forecasts, deadly flags, the boss-price HUD panel) and add hover/click enemy inspection with type + approximate stats per `AGENTS.md`.
